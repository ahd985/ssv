...
def run():
    ...
    be_data = {'%LDHR':['Loss of Decay Heat Removal', 1.0E-5],
                        '%LOSP':['Loss of Offsite Power', 1.0E-3],
                        '%ATWS':['Anticipated Transient Without SCRAM', 1.0E-4],
                        ...
                        'DCB-A-F':['Failure of DC Bus A',2.0E-5],
                        'DCB-B-F':['Failure of DC Bus B',2.0E-5],
                        'RHR-F':['Failure of Residual Heat Removal',1.0E-4],
                        'RCIC-F':['Failure of Reactor Core Isolation Cooling',4.0E-3],
                        'LPCI-A-F':['Failure of Low Pressure Coolant Injection A',5.0E-5],
                        'LPCI-B-F':['Failure of Low Pressure Coolant Injection B',5.0E-5],
                        'HPCI-F':['Failure of High Pressure Cooland Injection',1.0E-3]}

    cutset_freqs = [sum([be_data[be][1] for be in cutset]) for cutset in cutset_data]
    ...
    tab = ssv_model.add_element('table', 'cutset-report', data=report,
