...
var ElementContext = function () {
    ...
    _createClass(ElementContext, [{
        key: 'initialize_overlays',
        value: function initialize_overlays() {
        ...
        value: function set_font_scale() {
            var font_scale = parseFloat(d3.select(this.svg_selector).attr('viewBox').split(' ')[3]) / d3.select(this.svg_div_selector).node().getBoundingClientRect().height;
            ...
            for (var element_type in this.element_data) {
                for (var element in this.element_data[element_type]) {
                    ...
                    if (element_type == 'cell') {
                        this.elements.push(new Cell(this.uuid, element.ids, element.description, element.conditions, element.report_id));
                    } else if (element_type == 'line') {
                        this.elements.push(new Line(this.uuid, element.ids, element.description, element.conditions, element.report_id));
                    } else if (element_type == 'heatmap') {
                        this.elements.push(new Heatmap(this.uuid, element.ids, element.description, element.conditions, element.report_id));
                    } else if (element_type == 'toggle') {
                        this.elements.push(new Toggle(this.uuid, element.ids, element.description, element.conditions, element.report_id));
                    } else if (element_type == 'report') {
                        this.elements.push(new Report(this.uuid, element.ids, element.description, element.conditions, element.report_id));
                    } else if (element_type == 'table') {
                        this.elements.push(new Table(this.uuid, element.ids, element.description, element.conditions, element.report_id));
                    } else if (element_type == 'legend') {
                        this.elements.push(new Legend(this.uuid, element.ids, element.description, element.conditions, element.report_id));
                    }
                }
            }
        ...
        value: function update_elements(x) {
        ...
        key: 'toggle_pan_zoom',
        value: function toggle_pan_zoom() {
            var context = this;
        ...
        key: 'initialize_controls',


        // Initialize controls for ssv control bar
        value: function initialize_controls() {
            var context = this;
            ...
            d3.select(this.controls.play_selector).attr('ssv-id', this.uuid).on("click", function () {
                context.play();
            });

            // Clicking on zoom button toggles pan/zoom ability
            d3.select(this.controls.pan_zoom_selector).attr('ssv-id', this.uuid).on("click", function () {
                context.toggle_pan_zoom();
            });

            // Clicking on the speed button changes the speed of play
            d3.select(this.controls.speed_selector).attr('ssv-id', this.uuid).on("click", function () {
                ...
                d3.select(context.controls.speed_selector + ' span').html("<b>" + speed + 'x</b>');
            });
        }
    }, {
        key: 'initialize_pan_zoom',


        // Initializer of pan and zoom functionality
        value: function initialize_pan_zoom() {
            // Add border to svg to outline zoom/pan zone
            var ssv_svg = d3.select(this.svg_selector);
            ssv_svg.append('rect').attr('height', '100%').attr('width', '100%').style('fill', 'none');
...
var Element = function () {
    ...
    _createClass(Element, [{
        key: 'initialize_color_scales',
        value: function initialize_color_scales() {
            ...
            if (!sel.empty()) {
                ...
                for (var i = 0; i < this.conditions.length; i++) {
                    ...
                    if ("report" in condition && condition.report) {
                        ...
                        var section_label;
                        'section_label' in condition && condition.section_label != '' ? section_label = condition.section_label : section_label = 'Section';
                        ...
                        for (var j = 0; j < data_j_len; j++) {
                            ...
                            // Value text
                            var datum;
                            data_j_len > 1 ? datum = condition_data.map(function (i) {
                                return num_format(i[j]);
        ...
        value: function update_report(x) {
            ...
            var value_texts = parent.selectAll('.value-text').text(function (d) {
                return d[x];
            });
        }
    }, {
        key: 'initialize_pattern',


        // Function to initialize patterns for element
        value: function initialize_pattern(sel, pattern_id, prop_data, style_apply) {
            ...
            var pattern = defs.append('pattern').attr('id', pattern_id).attr('x', 0).attr('y', 0).attr('width', '100%').attr('height', '100%').attr('patternContentUnits', 'userSpaceOnUse').datum(prop_data);

            var pattern_data = pattern.selectAll().data(function (t) {
                return t[0];
            }).enter();
            ...
            pattern_data.each(function () {
                ...
                pattern.append('rect').attr('id', 'transition').attr('x', '0').attr('width', '100%').attr('height', '1px').attr('fill', '#FFFFFF');

                // Add overlays
                pattern.append('rect').attr('id', 'overlay').attr('x', '0').attr('width', '100%').attr('height', '100%');
            });

            var el = d3.select(sel);
            // Set to stroke or fill based on style_apply
            style_apply == 'fill' ? el.style('fill', 'url(#' + pattern_id + ')') : el.style('stroke', 'url(#' + pattern_id + ')');
        ...
        key: 'update_pattern',
        value: function update_pattern(x, sel, pattern_id) {
            var pattern = d3.select('#' + this.uuid + ' #' + pattern_id);

            // Update base pattern
            pattern.selectAll('#base').data(function (t) {
                return t[x];
            ...
            pattern.selectAll('#transition').data(function (t) {
                return t[x];
            }).transition().attr('y', function (d) {
                if (d[0] > 0) {
                    return d[0];
            ...
            // Update overlays
            pattern.selectAll('#overlay').data(function (t) {
                return t[x];
...
var Cell = function (_Element) {
    ...
    function Cell(uuid, cell_ids, cell_description, cell_conditions, cell_report_id) {
        ...
        return _possibleConstructorReturn(this, Object.getPrototypeOf(Cell).call(this, uuid, cell_ids, cell_description, cell_conditions, cell_report_id));
    }
                ...
                if (!_cell.patterns_initialized) {
                    ...
                    _cell.conditions.map(function (condition) {
                        ...
                        if (condition.type == 'background') {
                            ...
                            prop_data.push(condition.data.map(function (d) {
                                var order_val = (Math.min((d - condition.min_height) / (condition.max_height - condition.min_height)), 1);
...
var Line = function (_Element2) {
    _inherits(Line, _Element2);

    function Line(uuid, line_ids, line_description, line_conditions, line_report_id) {
        _classCallCheck(this, Line);

        return _possibleConstructorReturn(this, Object.getPrototypeOf(Line).call(this, uuid, line_ids, line_description, line_conditions, line_report_id));
    }
...
var Heatmap = function (_Element3) {
    _inherits(Heatmap, _Element3);

    function Heatmap(uuid, heatmap_ids, heatmap_description, heatmap_conditions, heatmap_report_id) {
        _classCallCheck(this, Heatmap);

        var _this3 = _possibleConstructorReturn(this, Object.getPrototypeOf(Heatmap).call(this, uuid, heatmap_ids, heatmap_description, heatmap_conditions, heatmap_report_id));

        _this3.conditions[0].data[0].constructor == Array ? _this3.conditions.num_sections = _this3.conditions[0].data[0].length : _this3.conditions[0].num_sections = 1;
    ...
    _createClass(Heatmap, [{
        ...
        value: function initialize() {
            ...
            _heatmap.selectors.map(function (d) {
                ...
                var x_section = g.selectAll().data(function (d) {
                    return d[0];
                }).enter().append('g').attr('class', 'x_section').attr('stroke', 'black').attr('stroke-width', '1px');

                // Add a color bin for every y slot across the axis
                x_section.selectAll('.bin').data(function (d) {
                    return d;
                }).enter().append('rect').attr('class', 'bin').attr('x', function (d, i) {
                    return x(i);
                    ...
                    return _heatmap.conditions[0].opacity;
                }).attr('height', bbox.height / initial_vals.length);

                x_section.each(function (d, i) {
                    d3.select(this).selectAll(".bin").attr("y", y(i));
                });
            });
        }
    }, {
        ...
        value: function update(x) {
            ...
            _heatmap.selectors.map(function (d) {
                ...
                var x_section = g.selectAll('.x_section').data(function (d) {
                    return d[x];
                });
                x_section.selectAll('.bin').data(function (d) {
                    return d;
            ...
            if (_heatmap.report_id) {
                _heatmap.update_report(x);
            }
        }
    }]);
...
var Toggle = function (_Element4) {
    _inherits(Toggle, _Element4);

    function Toggle(uuid, toggle_ids, toggle_description, toggle_conditions, toggle_report_id) {
        _classCallCheck(this, Toggle);

        var _this4 = _possibleConstructorReturn(this, Object.getPrototypeOf(Toggle).call(this, uuid, toggle_ids, toggle_description, toggle_conditions, toggle_report_id));

        _this4.data_initialized = false;
        return _this4;
    }
    ...
    _createClass(Toggle, [{
        ...
        value: function update(x) {
            ...
            if (_toggle.report_id) {
                _toggle.update_report(x);
            }
        }
    }]);
...
var Report = function (_Element5) {
    _inherits(Report, _Element5);

    function Report(uuid, report_ids, report_description, report_conditions, report_id) {
        _classCallCheck(this, Report);

        return _possibleConstructorReturn(this, Object.getPrototypeOf(Report).call(this, uuid, report_ids, report_description, report_conditions, report_id));
    }
...
var Table = function (_Element6) {
    _inherits(Table, _Element6);

    function Table(uuid, report_ids, report_description, report_conditions, report_id) {
        _classCallCheck(this, Table);

        return _possibleConstructorReturn(this, Object.getPrototypeOf(Table).call(this, uuid, report_ids, report_description, report_conditions, report_id));
    }
    ...
    _createClass(Table, [{
        ...
        value: function initialize_report() {
            ...
            if (!sel.empty()) {
                ...
                var table = parent.append('g').attr('transform', 'translate(' + x + ',' + y + ')').append('foreignObject').attr('id', 'ssv-table').attr("width", width).attr("height", 500).append('xhtml:table').attr('class', 'table').attr('id', table_id).datum(this.conditions[0].data);

                // Add table header
                table.append('thead').append('tr').selectAll().data(this.conditions[0].headers).enter().append('th').text(function (d) {
                    return d;
...
var Legend = function (_Element7) {
    _inherits(Legend, _Element7);

    function Legend(uuid, legend_ids, legend_description, legend_conditions, report_id) {
        _classCallCheck(this, Legend);

        return _possibleConstructorReturn(this, Object.getPrototypeOf(Legend).call(this, uuid, legend_ids, legend_description, legend_conditions, report_id));
    }
    ...
    _createClass(Legend, [{
        ...
        value: function initialize_report() {
            ...
            if (!sel.empty()) {
                ...
                var parent = d3.select(sel.node().parentNode);
                var legend = parent.append('g').attr('transform', 'translate(' + pos_x + ',' + pos_y + ')').append('g').attr('font-size', font_scale + 'em').attr('stroke', 'black').attr('stroke-width', '0.01em');
                ...
                // -- Append header text
                var legend_text = legend.append('text').text(this.description).attr('x', width / 2).attr('y', '1em').attr('text-anchor', 'middle').attr('font-size', header_em.toString() + 'em');
                ...
                keys.enter().append('rect').attr('x', function (d, i) {
                    return x(i);
                }).attr('y', (1 + margin) * legend_text_height).attr('width', function (d, i) {
                    return x(i + 1) - x(i);
                }).attr('height', legend_text_height).attr('fill', function (d) {
                    return d;
                }).style('fill-opacity', this.conditions[0].opacity).attr('font-size', header_em.toString() + 'em');

                // -- Append value text immediately below the left-most area of every color rect
                keys.enter().append('text').text(function (d) {
                    return color_scale.invertExtent(d)[0].toFixed(0);
                }).attr('x', function (d, i) {
                    return x(i);
                }).attr('y', legend_text_height * (2 + margin + label_em / header_em * (1 + margin))).attr('text-anchor', 'middle').attr('font-size', label_em.toString() + 'em');
            }
...
//# sourceMappingURL=ssv-compiled.js.map